...
class ChecklistAPI extends MysqlDB
{
    ...
    function getItems($tpl_id, $is_active = false)
    {
        ...
        exit;
        */
        $sql = "SELECT a.ID, b.Item, b.ItemDesc, b.IsDeleted, a.ItemCode, a.TplID, b.ID as MetaID FROM checklist_item a, checklist_meta b, checklist_tpl c where a.ItemCode = b.ItemCode and a.TplID = {$tpl_id} and a.tplid = c.id ";
        if ($is_active) {
        ...
        while ($this->fetch()) {
            ...
            $rtn[$this->ID]['del'] = $this->IsDeleted;
            $rtn[$this->ID]['meta_id'] = $this->MetaID;
        }
        //var_dump($rtn);
        return $rtn;
    }
    ...
    function getTpls($tpl_id = 0, $active = false, $tplids = array())
    {
        ...
        $sql .= " order by visaname+0, Subject";
        //echo $sql;
        $this->query($sql);
        while ($this->fetch()) {
    ...
    function getApp($type, $appid, $html_break = true)
    {
        ...
        $sql = "select a.ID, a.Received, if(a.ExItem <> '', ExItem, b.Item) as Title, if(a.ItemCode = '', '', b.ItemDesc) as Remark, b.ID as MetaID  from checklist_app a left join checklist_meta b on (a.ItemCode = b.ItemCode) where a.type = '{$type}' and a.Appid = {$appid} order by a.Rank, a.ID";
    ...
    function addAppItems($type, $appid, $item, $received = '')
    {
        ...
        if ($item != '') {
            ...
            $sql = "select max(Rank) as max_rank from checklist_app where Type = '{$type}' and AppID = {$appid} ";
            $this->query($sql);
            $row = $this->fetch_array();
            $rank = isset($row['max_rank']) ? $row['max_rank'] : 0;
            $rank++;

            $item = addslashes($item);
            $sql = "insert into checklist_app (Type, AppID, ExItem, Received, Rank) values ('{$type}','{$appid}','{$item}','{$received}', '{$rank}')";
            return $this->query($sql);
        }
        return false;
    }
