...
<table border="0" cellpadding="1" cellspacing="1" width="100%">
    ...
    <tr align="left" class="totalrowodd">
        <td class="border_1" colspan="3">
            ...
            &nbsp;
            <button type="button" onclick="do_checklist('apply_tpl')" {if count($tpl_arr)==0}
                disabled{/if}><strong>Create with a template</strong></button>
            &nbsp;&nbsp;
...
<table border="0" cellpadding="1" cellspacing="1" width="100%" id="tbl_checklist">
    <tr class="greybg">
        <td colspan="5" class="whitetext" align="center">
            ...
            {/if}
            {if count($app_arr) > 0}
            <a href="/scripts/checklist_pdf.php?cl_typ={$cl_typ}&cl_appid={$cl_appid}" , target="_blank">PDF
                Download</a>
            {/if}
        </td>
    </tr>
    ...
    <tr align="center" class="roweven" id="app_item_{$id}">
        ...
        <td width="5%"><input type="checkbox" id="app_chk_{$id}" {if $v.rcd !='0000-00-00' } checked {/if}
                onChange="mark_checklist({$id},this)"></td>
        <td>
            {if $v.rmk != ""}<span class="ui-icon ui-icon-help" title="{$v.rmk}"></span>&nbsp;{/if}
            {if isset($attachments[$id])}
            <span class="ui-icon ui-icon-document" onClick="openinSatff('attachment_{$id}')"></span>
            <div style="display:none; float:inherit; position:absolute; background-color:#FFFFCC;width::400px"
                id="attachment_{$id}">
                <ul>
                    {foreach item=attach_info from=$attachments[$id]}
                    <li>
                        <a href="/download/{$attach_info.url}" target="_blank">{$attach_info.file}</a>
                    </li>
                    {/foreach}
                </ul>
                <span style="font-weight:bolder; cursor:pointer;"
                    onClick="$('#attachment_{$id}').css('display', 'none')">&times;</span>
            </div>
            {/if}
        </td>
    </tr>
