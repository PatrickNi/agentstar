...
<table width="100%" cellpadding="3" cellspacing="1" border="0" align="left">
  ...
  <tr align="right" class="roweven" >
    ...
    <td>
          ...
          {foreach key=catename item=catenum from=$courseprocs[$week].apocnt_st}
              ...
              <br/><span onClick="openinSatff('d2_{$week}_{$cateidx}');"  style="text-decoration:underline; cursor:pointer;">({$catenum}) {$catename}</span> 
     ...
     <td >
           ...
           {foreach key=catename item=catenum from=$courseprocs[$week].reocnt_st}
              {assign var="cateidx" value=$cateidx+1}
              <br/><span onClick="openinSatff('d3_{$week}_{$cateidx}');"  style="text-decoration:underline; cursor:pointer;">({$catenum}) {$catename}</span>
    ...
    <td>
        ...
        {foreach key=catename item=catenum from=$courseprocs[$week].reccnt_st}
             {assign var="cateidx" value=$cateidx+1}
            <br/><span onClick="openinSatff('d4_{$week}_{$cateidx}');"  style="text-decoration:underline; cursor:pointer;">({$catenum}) {$catename}</span>
  ...
  <tr align="right" class="roweven" >
    <!-- Visa Consultant -->
    <td>
      <span onClick="openinSatff('d6_{$week}');" style="text-decoration:underline; cursor:pointer;">{$visavisits[$week].pcnt}</span>
    ...
    <!-- Consultant Fee -->
    <td>
      <span onClick="openinSatff('d66_{$week}');" style="text-decoration:underline; cursor:pointer;">{$visavisits[$week].totalcfee}</span>
    ...
    <!-- Agreement Signed -->
    <td>
      <span onClick="openinSatff('d7_{$week}');" style="text-decoration:underline; cursor:pointer;">{$visaagrees[$week].fee|string_format:"%.2f"}&nbsp;($0:{$visaagrees[$week].sign0}/paid:{$visaagrees[$week].sign1})</span>
...
{if $visaapplies.$week.art_apply.clients > 0}
  ...
  <tr align="right" class="roweven" >
    <td>
          ...
          <hr>
          expt.paperwork:
          <span onClick="openinSatff('d991_{$week}');" style="text-decoration:underline; cursor:pointer;">({$visareviews[$week].cnt_notpaperwork})</span>
          {if $staffid != 105}
            $ {$visareviews[$week].total_profit_notpaperwork}
          {/if}
          {if $staffid != 105}
            <div style="display:none; float:inherit; position:absolute; background-color:#FFFFCC;width::400px" id="d991_{$week}">
              <ul>
                {foreach key=id item=name from=$visareviews[$week].pname}
