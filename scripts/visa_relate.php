...
if (isset($_POST['bt_name']) && strtoupper($_POST['bt_name']) == "SAVE"){
	...
	if($rid > 0){
		$o_v->setVisaRelate($rid, $process_name, $process_order);
	}else{
		$o_v->addVisaRelate($cat_id, $sub_id, $process_name, $process_order);
	}
	$rid = 0;
	$isNone = "none";	
...
# format array
$relate_arr = $o_v->getVisaProcessArr($cat_id, $sub_id);
...
# get sublcass name
$sub_name = $o_v->getSubclassName($cat_id, $sub_id);
