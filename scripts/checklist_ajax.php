...
function getAppAttachements($app_arr, $cl_type)
{
    $o_g = new GeicAPI(__DB_HOST, __DB_USER, __DB_PASSWORD, __DB_DATABASE, 1);
    $attachment_type = 'checklist-' . $cl_type;
    $rtn = [];
    foreach ($app_arr as $id => $v) {
        $tmp = $o_g->getAttachment($v['meta_id'], $attachment_type);
...
if (count($app_arr) == 0 && $cl_act == '') {
    ...
    if (!$cl_tplid && $cl_act == 'apply_tpl') {
        ...
        $app_arr = $o_c->getApp($cl_typ, $cl_appid);
        $attachments = getAppAttachements($app_arr, $cl_typ);
        $o_tpl->assign('app_arr', $app_arr);
        $o_tpl->assign('attachments', $attachments);
        $output = $o_tpl->fetch('checklist_ajax.tpl');
    }
